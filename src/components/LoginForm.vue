<template>
  <a-form>
    <a-form-item v-if="loginShow" :label-col="login.labelCol" :wrapper-col="login.wrapperCol">
      <a-form
        :model="login"
        @finish="loginFinish"
      ...
      >
        <a-form-item label="手机" name="phone">
          <a-input
            v-model:value="login.phone" placeholder="请输入手机号">
            <template #prefix>
        ...
        <a-form-item label="密码" name="password">
          <a-input
            v-model:value="login.password" type="password" placeholder="请输入密码">
            <template #prefix>
    ...
    <a-form-item>
      <div style="display: flex">
        <a-space align="center" direction="vertical" style="margin:0 auto" size="large">
          <a-checkbox v-model:checked="this.license.agree">
            我同意<a @click="this.license.show=true" style="">《用户许可协议》</a>
          </a-checkbox>
          <a-space align="center" size="large" style="margin:0 auto">
            <a-button @click="loginClick" :type="this.loginShow?'primary':'default'">登录</a-button>
            <a-button @click="registerClick" :type="!this.loginShow?'primary':'default'">注册</a-button>
          </a-space>
        </a-space>
      </div>
    </a-form-item>
  ...
  <a-modal
    width="60%"
    v-model:visible="license.show" title="用户许可协议" @ok="handleOk" @cancel="handleCancel"
    :closable="false"
    :maskClosable="false"
    ok-text="同意"
    cancel-text="不同意">
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      this.license.show = false;
      this.license.agree = true;
    },
    handleCancel() {
      this.license.show = false;
      this.license.agree = false;
    },
    validatorPhoneNumber(rule, value, callback) {
      if (!value) {
        return callback(new Error("请输入手机号"));
      } else {
        let isPhone = /^1[3456789]\d{9}$/.test(value);
        if (isPhone) {
          callback();
        } else {
